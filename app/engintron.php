...
define('ENGINTRON_STATE', trim(file_get_contents("/usr/local/engintron/state.conf")));
=======
define('PLG_VERSION', '1.13.0');
define('PLG_BUILD', 'Build 20200328');
define('NGINX_VERSION', trim(str_replace('nginx version: nginx/', '', shell_exec('nginx -v 2>&1'))));
>>>>>>> master
...
function execute($act) {
	...
	switch($act){
		...
		case "httpd_status":
			if(file_exists("/usr/local/engintron/state.conf") && ENGINTRON_STATE=="on"){
...
switch($op) {
	...
	case "engintron_res":
		$ret = shell_exec("bash /usr/local/engintron/engintron.sh res 2>&1");
		break;

	case "engintron_resall":
		$ret = shell_exec("bash /usr/local/engintron/engintron.sh resall 2>&1");
		break;
