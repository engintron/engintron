...
But there is one specific drawback to using Nginx in reverse caching proxy mode as installed by Engintron: if you site allows user generated content (e.g. a forum), then Nginx will not work properly out-of-the-box. When you got users that login in the frontend and are able to browse user-specific content or upload/generate their own content (for example in a forum website), you need to write additional Nginx rules to make sure you exclude these user pages (e.g. the login/logout pages, the user's dashboard and so on). Additionally you need to setup cookie-based rules so that Nginx persistently "understands" when a user is logged in and therefore does not cache any page output for them. This way, the user pages will never by cached by Nginx and your users can interact with your site as they did before you installed Engintron. 
